...
#### Functions to be used in the window : 
...
def url_scrapper():
    print(website_url.get())
    local_scrapper  = scrapping_file.Scrapper(website_url.get() , 1)
    message  =  local_scrapper.get_text()
    print(message)
...
scrap_button  = ctk.CTkButton(master=window , text="Start Scrapping" , command=url_scrapper)
